...
from .forms import (ArtistForm, AlbumForm, ReviewForm, CustomLoginForm,
...
def change_password_with_security_questions(request):
    ...
    if request.method == 'POST':
        ...
        if 'security_answers' in request.POST:
            ...
            # Regular password change form submission (after security verification)
            form = PasswordChangeForm(request.user, request.POST)
            if form.is_valid():
                user = form.save()
                update_session_auth_hash(request, user)
...
class SecurityQuestionPasswordResetView(View):
    """
    Handle password reset using security questions - step 1: username verification
    """
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                all_questions = [
                    (security_questions.question_1, security_questions.answer_1, 1),
                    (security_questions.question_2, security_questions.answer_2, 2),
                    (security_questions.question_3, security_questions.answer_3, 3),
                    (security_questions.question_4, security_questions.answer_4, 4),
                    (security_questions.question_5, security_questions.answer_5, 5),
                ]
                ...
                messages.error(self.request, 'User not found. Please check the username.')
            except SecurityQuestion.DoesNotExist:
                messages.error(self.request, 'Security questions not found for this user. Please contact admin.')
...
def superuser_required(function=None):
    ...
    """
    actual_decorator = user_passes_test(
        lambda u: u.is_authenticated and u.groups.filter(name='Superuser').exists(),
        login_url='/accounts/login/'
    )
    if function:
        return actual_decorator(function)
...
def staff_required(function=None):
    ...
    """
    actual_decorator = user_passes_test(
        lambda u: u.is_authenticated and u.groups.filter(name='Staff').exists(),
        login_url='/accounts/login/'
    )
    if function:
        return actual_decorator(function)
...
def delete_album_frontend(request, album_id):
    ...
    return render(request, 'chinook_app/delete_confirm.html', {'object': album})
...
def delete_artist_frontend(request, artist_id):
    ...
    return render(request, 'chinook_app/delete_confirm.html', {'object': artist})

@method_decorator(csrf_protect, name='dispatch')
class SecurityQuestionVerificationView(View):
    """
    Handle password reset using security questions - step 2: question verification
    """
    ...
    def get(self, request):
        ...
        form = SecurityQuestionVerificationForm(security_questions=security_questions)
    ...
    def post(self, request):
        ...
        form = SecurityQuestionVerificationForm(request.POST, security_questions=security_questions)
        ...
        if form.is_valid():
            try:
                ...
                for i, question_data in enumerate(security_questions):
                    user_answer = form.cleaned_data[f'answer_{i+1}'].lower().strip()
                    correct_answer = question_data['correct_answer'].lower().strip()
                    
                    if user_answer == correct_answer:
                        correct_answers += 1
                
                if correct_answers >= 2:  # Require both answers to be correct
                    self.request.session['questions_verified'] = True
                    self.request.session['verified_user_id'] = user.id
                    messages.success(self.request, 'Security questions verified successfully! You can now reset your password.')
                    return redirect('password_reset_from_questions')
                else:
                    messages.error(self.request, 'Incorrect answers. Please try again or contact admin.')
...
class QuestionBasedPasswordResetView(View):
    """
    Handle password reset using security questions - step 3: password reset
    """
    template_name = 'account/password_reset_from_questions.html'
    
    def get(self, request):
        if not self.request.session.get('questions_verified'):
            messages.error(self.request, 'Please verify security questions first.')
            return redirect('security_question_reset')
...
class CustomPasswordResetView(PasswordResetView):
    """
    Override the default allauth password reset view to redirect to security questions
    """
    form_class = CustomResetPasswordForm

    def form_valid(self, form):
        # Redirect to security question flow instead of sending email
        username = form.cleaned_data.get('email')  # Using email field for username
        if username:
            try:
                user = User.objects.get(username=username)
                self.request.session['reset_username'] = username
...
def index(request):
    ...
    stats = {
        ...
        'recent_albums': Album.objects.select_related('ArtistId').order_by('-AlbumId')[:3],
        'recent_tracks': Track.objects.select_related('AlbumId', 'AlbumId__ArtistId').all()[:15],
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.success(request, f'Artist "{artist_name}" added successfully with ID: {artist_id}!')
                return redirect('all_artists')
        ...
        if form.is_valid():
            ...
            try:
                with connection.cursor() as cursor:
                    ...
                    cursor.execute(
                        'INSERT INTO "Album" ("Title", "ArtistId") VALUES (%s, %s) RETURNING "AlbumId"',
                        [album_title, artist_id]
                    )
                    album_id = cursor.fetchone()[0]
                
                messages.success(request, f'Album "{album_title}" added successfully!')
                return redirect('all_albums')
        ...
        if 'select_artist' in request.POST:
            ...
            if artist_id and new_name:
                try:
                    with connection.cursor() as cursor:
                        cursor.execute(
                            'UPDATE "Artist" SET "Name" = %s WHERE "ArtistId" = %s',
                            [new_name, artist_id]
                        )
                    
                    messages.success(request, f'Artist updated successfully to "{new_name}"!')
                    return redirect('all_artists')
        ...
        if 'select_album' in request.POST:
            ...
            if album_id and new_title:
                try:
                    with connection.cursor() as cursor:
                        cursor.execute(
                            'UPDATE "Album" SET "Title" = %s WHERE "AlbumId" = %s',
                            [new_title, album_id]
                        )
                    
                    messages.success(request, f'Album updated successfully to "{new_title}"!')
                    return redirect('all_albums')
    ...
    if request.method == 'POST':
        if 'select_artist' in request.POST:
            ...
            if artist_id:
                ...
                # Check if artist has albums
                if Album.objects.filter(ArtistId=artist_id).exists():
                    error = "Cannot delete artist with existing albums. Please delete the albums first."
    ...
    if request.method == 'POST':
        if 'select_album' in request.POST:
            ...
            if album_id:
                ...
                # Check if album has tracks
                if Track.objects.filter(AlbumId=album_id).exists():
                    error = "Cannot delete album with existing tracks. Please delete the tracks first."
...
def delete_review(request, review_id):
    ...
    return render(request, 'chinook_app/delete_review.html', {'review': review})
